...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'income_sources_monthly_amount_nonnegative'
  ) THEN
    ALTER TABLE public.income_sources
      ADD CONSTRAINT income_sources_monthly_amount_nonnegative
...
BEGIN
  ...
  LOOP
    IF NULLIF(BTRIM(item->>'name'), '') IS NOT NULL THEN
      source_amount := COALESCE(NULLIF(item->>'monthlyAmount', '')::numeric, 0);

      INSERT INTO public.income_sources (household_id, name, description, monthly_amount)
      VALUES (
        new_household_id,
        BTRIM(item->>'name'),
